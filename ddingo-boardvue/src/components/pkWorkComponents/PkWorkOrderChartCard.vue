...
export default {
  ...
  methods: {
    initData() {
      var data = {
        fabCode: this.chartDFDataValue.fabCode,
        itemCode: this.chartDFDataValue.itemCode
      };
      var result = [];
      var specResult = [];
      axios
          .post("/api/get/pkworkitemlist", data)
          .then(response => {
    ...
    SetChartData(result) {
      ...
      for (var i = 0; i < result.length; i++) {
        ...
        if (i == 0) {
          ...
          if (result[i].itemCode != result[i - 1].itemCode) {
            data = {
              label: result[i].itemName,
              backgroundColor: chartColor,
              borderColor: chartColor,
            ...
            for (var index = 0; index < fomatChartData.length; index++) {
              if (fomatChartData[index].label == result[i].itemName) {
